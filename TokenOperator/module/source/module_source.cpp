...
module_source::module_source(std::pair<object*, bool> source, module_source_requirement* requirements, size_t requirement_num, scope_path path_to_set, uint64_t ID) : value<std::pair<smart_pointer<object>, bool>>(ID), init_status{ false, false }, requirement_num(requirement_num), requirements(new module_source_requirement[requirement_num]), path_to_set(path_to_set) {
	v = source;
